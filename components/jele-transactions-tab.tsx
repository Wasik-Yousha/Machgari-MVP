...
export function JeleTransactionsTab() {
  ...
  const [journeyModalShownFor, setJourneyModalShownFor] = useState<Set<number>>(new Set())
  ...
  const handleCompleteTransaction = (transactionId: number) => {
    ...
    // Show journey modal immediately after completing transaction
    setTimeout(() => {
      setShowJourneyModal(true)
      setJourneyModalShownFor(prev => new Set([...prev, transactionId]))
    }, 500)
  }

  // Handle journey modal close
  ...
  const handleStartJourney = () => {
    if (selectedTransaction) {
      setShowJourneyModal(true)
      setJourneyModalShownFor(prev => new Set([...prev, selectedTransaction.id]))
    }
  }
              ...
              <CardContent className="space-y-6">
                ...
                {selectedTransaction.status === "accepted" && (
                  <div className="space-y-4">
                    ...
                    <Button className="w-full bg-purple-600 hover:bg-purple-700" onClick={() => handleCompleteTransaction(selectedTransaction.id)}>
                  ...
                  <div className="space-y-4">
                    <div className="bg-purple-50 dark:bg-purple-900/20 p-3 rounded-lg">
                      <div className="flex items-start">
                        ...
                        <div>
                          <p className="text-sm text-purple-700 dark:text-purple-300 font-medium">লেনদেন সম্পন্ন হয়েছে</p>
                          <p className="text-lg font-semibold mt-1">চূড়ান্ত মূল্য: {formatCurrency(selectedTransaction.finalPrice || selectedTransaction.currentOffer)}</p>
                          <p className="text-xs text-gray-500 mt-1">আপনার মাছ আড়তে পরিবহন করার জন্য যাত্রা শুরু করুন</p>
                        </div>
                      </div>
                    </div>
                    
                    <Button className="w-full bg-blue-600 hover:bg-blue-700" onClick={handleStartJourney}>
      ...
      {selectedTransaction && (
        <JourneyStartModal 
          ...
          transaction={{
            ...
            fishName: selectedTransaction.fishName,
            quantity: selectedTransaction.quantity,
            finalPrice: selectedTransaction.finalPrice,
            arotdarName: selectedTransaction.arotdarName,
            arotdarLocation: "ঢাকা বাজার", // Default location for demo
            arotdarPhone: "+৮৮০১৭১২৩৪৫৬৭৮" // Default phone for demo
          }}
        />
      )}
    </>
